...
- `impl Trait + use<'a, T>` - use<> can appear anywhere in bounds (per Issue #125836)
...
### Objective
Add `use<..>` syntax to the parser grammar as a **syntactic bound** per [Issue #125836](https://github.com/rust-lang/rust/issues/125836).

### Important Design Decision

The Rust language team decided that `use<..>` is a **bound**, not a prefix. This means:
- `impl use<T> + Trait` - valid (use<> as first bound)
- `impl Trait + use<T>` - valid (use<> after other bounds)
- `impl use<T> Trait` - **invalid** (missing `+` between bounds)

This differs from early RFC 3617 drafts which showed `use<>` as a prefix immediately after `impl`.
...
**1.2 Modify PolyboundInner** (line ~500):

```bnf
// Before
private PolyboundInner ::= [ ForLifetimes | '?' | TildeConst | ConstBound | BracketConst ] Bound

// After - make UseBoundsClause a valid bound alternative
private PolyboundInner ::= UseBoundsClause | [ ForLifetimes | '?' | TildeConst | ConstBound | BracketConst ] Bound
```

**Note:** `ExplicitTraitTypeInner` remains unchanged:
```bnf
private ExplicitTraitTypeInner ::= (impl | dyn) Polybound ('+' Polybound)* { pin = 1 }
```

**1.3 Handle nullable `RsPolybound.bound`**

Since a `Polybound` can now contain either a `Bound` or a `UseBoundsClause`, the generated
`RsPolybound.getBound()` method returns `RsBound?` (nullable). All code accessing `.bound`
on `RsPolybound` must be updated to handle this:

Files requiring updates:
- `RsErrorAnnotator.kt` - use `it.bound?.traitRef`
- `RsSyntaxErrorsAnnotator.kt` - use `it.bound?.lifetime`
- `RsDocumentationProvider.kt` - use `bound?.lifetime ?: bound?.traitRef`
- `AddMissingSupertraitImplFix.kt` - use `it.bound?.traitRef`
- `RsGenericParameterInfoHandler.kt` - use `it.bound?.traitRef`
- `RsNeedlessLifetimesInspection.kt` - use `.mapNotNull { it.bound }`
- `RsPsiRenderer.kt` - check `bound != null` before accessing
- `Utils.kt` - use `it.bound?.traitRef`
- `RsExtractFunctionConfig.kt` - use `polybound.bound?.traitRef`
- `RsTraitItem.kt` - use `it.bound?.traitRef`
- `ImplLookup.kt` - use `it.bound?.traitRef`
- `TyLowering.kt` - use `it.bound ?: return@mapNotNull null`
- `TypeInference.kt` - use `bound.bound?.traitRef`
- `TyTypeParameter.kt` - use `it.bound?.traitRef` and `it.bound?.lifetime`
- `RsPrimitiveTypeImplsTest.kt` (test) - use `it.bound?.traitRef`
...
- [ ] Parser accepts `impl Trait + use<'a, T>` syntax (what stdlib uses)
- [ ] Parser accepts `impl use<> + Sized` (empty)
...
- [ ] All nullable bound usages updated

### Estimated Effort
**Medium** - Grammar changes plus nullable handling in ~14 files

### Checkpoint
Run: `./gradlew :test --tests "org.rustSlowTests.RsCompilerSourcesPerfTest"`
...
```kotlin
// Note: Since use<> is now a Polybound, we search within polyboundList
...
object RsUseBoundsCompletionProvider : RsCompletionProvider() {
    ...
    override fun addCompletions(
        ...
        // Note: Since use<> is now inside Polybound, traverse: UseBoundsClause -> Polybound -> TraitType
        val polybound = useBoundsClause.parent as? RsPolybound ?: return
        val traitType = polybound.parent as? RsTraitType ?: return
...
| `src/test/resources/org/rust/lang/core/parser/fixtures/complete/use_capturing.txt` | **Gold file** (auto-generated) |
| `src/test/kotlin/org/rust/lang/core/parser/RsCompleteParsingTestCase.kt` | Add test method |

**Important:** The fixture file alone is not sufficient. You must also:
1. Add `fun \`test use capturing\`() = doTest(true)` to `RsCompleteParsingTestCase.kt`
2. Run the test to generate the gold file `use_capturing.txt`

### Test Content

**Note:** Per Issue #125836, `use<>` is a syntactic bound and requires `+` to separate from other bounds.

```rust
// =============================================================================
// Basic use bounds (RFC 3617: use<> as a syntactic bound)
// =============================================================================
...
// =============================================================================
// use<> appearing after trait bounds (Issue #125836: use<> is a bound)
// =============================================================================
fn use_after_trait<T>(_: T) -> impl FnMut(T) + use<T> { |_| () }
fn use_after_multiple_traits<'a, T>() -> impl Clone + Send + use<'a, T> {}
...
- [ ] `RsTraitType.useBoundsClause` is present in AST (via polyboundList)
- [ ] Parser accepts `impl Trait + use<T>` syntax (use<> after bounds)
- [ ] Test method exists in `RsCompleteParsingTestCase.kt`
- [ ] Gold file `use_capturing.txt` is generated and committed
...
### Checkpoint
```bash
./gradlew :test --tests "org.rust.lang.core.parser.RsCompleteParsingTestCase.test use capturing"
```
...
### Design Note: Trait Bounds in Tests

**Why tests use `impl use<'a>` without explicit trait bounds:**

In valid Rust, `impl Trait` requires at least one trait bound (e.g., `impl use<'a> Sized`).
However, for plugin type inference tests:

1. **No stdlib available**: Without the standard library, `Sized` cannot be resolved, so
   `type.knownItems.Sized` returns `null` and the trait list remains empty.
2. **Plugin grammar flexibility**: The grammar allows `impl use<>` without traits for
   error recovery, enabling the plugin to provide IDE features even for incomplete code.
3. **Focused testing**: Tests verify capture clause parsing and rendering without
   requiring trait resolution infrastructure.

The type rendering correctly outputs `impl use<'a>` when the trait list is empty,
which is the expected behavior for these test scenarios.
...
""")
// Note: Captures are stored internally but not rendered (hasExplicitCaptures = false)
...
""")
// Note: No captures stored (legacy behavior)
```

### Success Criteria

- [ ] Explicit `use<..>` captures resolve correctly
- [ ] Edition 2024 implicit captures work (stored internally)
- [ ] Edition 2021 has no implicit capture tracking
- [ ] Type rendering shows captures only when `hasExplicitCaptures = true`
...
### Checkpoint
```bash
./gradlew :test --tests "org.rust.lang.core.type.RsExpressionTypeInferenceTest"
```
...
- [ ] Modify `PolyboundInner` to include `UseBoundsClause` as a bound alternative
- [ ] Run `./gradlew :generateParser`
- [ ] Verify basic parsing works
- [ ] Update all code accessing nullable `RsPolybound.bound` (~14 files)

### Phase 2: PSI Extensions
- [ ] Create `RsUseBoundsClause.kt`
- [ ] Add `useBoundsClause` accessor to `RsTraitType.kt` (searches polyboundList)
...
- [ ] Update `RsPsiRenderer.kt` to handle use<> bounds
- [ ] Verify in development IDE

### Phase 5: IDE Completion
- [ ] Add `use` keyword completion pattern
- [ ] Create `RsUseBoundsCompletionProvider.kt` (updated for Polybound parent)
- [ ] Register provider in `RsCompletionContributor.kt`
- [ ] Test completion in development IDE

### Phase 6: Parser Tests
- [ ] Create `use_capturing.rs` test fixture (updated with `+` syntax)
- [ ] Run parser tests
- [ ] Verify PSI structure with gold files
- [ ] Verify `RsCompilerSourcesPerfTest` passes with stdlib
